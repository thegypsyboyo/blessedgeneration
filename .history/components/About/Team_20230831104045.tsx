                    ...
                    {duplicatedData?.map((item, index) => (
                        <SwiperSlide key={index}  className='h-full w-full relative !bg-white'>
                            <div className='single w-full h-full relative block group'>
                                  ...
                                  <div className="absolute flex flex-col top-0 left-0 transition-all duration-500  ">
                                    ...
                                    <ul className="max-w-[40px] flex-col transition-all duration-500 h-full transform translate-y-[-160px] flex justify-center group-hover:translate-y-[0px]">
                                        ...
                                        <li key={index} className=" w-[40px] h-[40px] flex ">
                                          <Link href={social.url} rel="noreferrer noopener" target="_blank" className={`${social.platform === "facebook" ? "hover:bg-blue-800":""} ${social.platform === "instagram" ? "hover:[#F56040]":""}  ${social.platform === "linkedin" ? "hover:bg-blue-400":""} ${social.platform === "youtube" ? "hover:[#c4302b]":""} bg-[#489f10] hover:bg-redDark text-white cursor-pointer w-full h-full flex items-center justify-center z-[99999]`}>
                                ...
                                <div className="w-full h-[150px] bg-yellowColor p-4">
                                  <p className="text-white font-bold font-cav">{item.position}</p>
                                  <Link href={`/team/team-details/${item.slug}`}  className="text-white text-[20px] font-bold font-nan">{item.name}</Link>
                                </div>
                            </div>
                        </SwiperSlide>
                    ))}
