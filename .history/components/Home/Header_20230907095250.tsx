      ...
      <div className='swiper-container relative w-full h-full z-100 flex transition-transform duration-500 ease-in-out box-content'>
        {data.map((item, index) =>(
          ...
          >
            ...
            <div className="w-full h-[120vh] text-center bg-cover bg-center" style={{backgroundImage: `url(${item.img})`}}>
              <div className='slide-layer'/>
              <div className="inner">
                <h1 className="font-bold my-0 mx-auto text-white text-[37px] md:text-[50px] lg:text-[64px] ">{item.title}</h1>
                <p className="text-light text-[16px] leading-[24px] mt-[20px] my-auto mb-[30px] max-w-[640px]">{item.description}</p>
                <AnimatedButton className='bg-yellowColor' link='/donate' text='discove more' style='!mt-0 !bg-redDark'/>
              </div>
            </div>
