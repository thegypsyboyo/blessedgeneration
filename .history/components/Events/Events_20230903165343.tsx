...
interface EventWithStatus extends EventsProps {
  ended: boolean;
}
  ...
  const ndata: EventWithStatus[] = [
    {
      ...
      timeStamp: "2023-09-01T09:00:00", // Set the event's end time
      ended: false, // Initially assume the event has not ended
    },
    // Add more events here...
  ];

  const duplicatedData = [...ndata];
  const [data, setData] = useState<EventWithStatus[]>([]);


  useEffect(() => {
    const currentTime = new Date().getTime();

    // Update the 'ended' status for each event based on the current time and timestamp
    const updatedData = data?.map((event:any) => ({
      ...event,
      // Convert the timestamp string to a Date object for comparison
      ended: new Date(event.timeStamp).getTime() <= currentTime,
    }));
