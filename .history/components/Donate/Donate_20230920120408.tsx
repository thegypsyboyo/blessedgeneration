...
import { CausesItem, DonationProps } from '@/typings'
...
const Features = ({causesitem}:Props) => {

    const [causesItem, setCausesItem] = useState(causesitem); 
    const [filteredCauesItems, setFilteredCausesItems] = useState(causesitem);
    const [activeFilter, setActiveFilter] = useState<string>("All");

    
  useEffect(() => {
    setCausesItem(causesitem)
    ...
    <section className='relative block pt-[60px] mt-[0px] pb-[90px] px-0 w-full'>
        <Layout className='flex flex-col items-center' >
            <div className="relative block -mt-[4px] mb-[46px] text-center">
                <span className="text-primaryBase text-[24px] font-[700] leading-[24px] mb-[10px] font-cav capitalize">some of our causes</span>
                <h2 className="m-0 text-orangeDefault lg:text-[50px] lg:leading-[60px] text-[35px] leading-[40px] -tracking-[0.04em] font-[500] lg:max-w-[650px] sm:max-w-[500px] mt-[6px] capitalize">Find our popular cause and help us reach our goal </h2>
            </div>
            <nav className=''>
              <div className='flex flex-wrap items-center justify-center space-x-3 mb-[35px] text-center'>
                {['All', 'education', 'medicine', 'food & water', 'Sponsorship'].map((category) => (
                  <button
                  key={category}
                ...
                {filteredCauesItems.map((item, index) => (
                    <div className="transition-all duration-500 w-full px-[15px] p-[15px] lg:max-w-[33.3333%] md:max-w-[50%] flex-grow-0 flex-shrink-0 group flex items-start" key={index}>
                        <div className="relative mb-[30px] flex flex-col items-center justify-items-center ">
                            <div className="relative block overflow-hidden z-[1] rounded-tl-lg rounded-tr-lg">
                                ...
                                <Image src={urlFor(item.image).url()} alt='main-image' width={800} height={800} className='w-full h-full object-cover transform scale-[1] transition duration-500 ease group-hover:scale-[1.05] rounded-tl-lg rounded-tr-lg'/>
                            ...
                            <div className="relative block bg-white shadow-main border-solid border-borderColor pt-[22px] pb-[30px] px-[30px] border-t-0">
                                <h3 className="text-[18px] leading-[26px] font-[800] -tracking-[0.04em] mb-5">
                                    <Link href={`/donate/donation-info/${item.slug.current}`} className='text-redLight transition-all duration-500 hover:text-darkBlue'>{item.title}</Link>
                                </h3>
                                <div className="relative block w-full pt-[49px] px-[29px] pb-[20px] bg-borderColor rounded-[20px] border-solid border-borderColor z-[1]">
                                    <div className="relative w-full h-[7%] bg-white rounded-[5px] mb-[3px]">
                                        <div className="relative block h-[7px] rounded-[5px] bg-redLight transition-all duration-[1500ms] " style={{width: item.estimatePercentage}}>
                                    ...
                                    <div className="flex flex-wrap justify-between items-center">
                                        <p className="text-[14px] text-primaryText font-[400] m-0">
                                            <span className="text-[14px] text-primaryDark font-[700] leading-[28px] mr-[5px]">Ksh.{item.amountDonated}</span>
                                            Raised
                                        </p>
                                        <p className="text-[14px] text-primaryText font-[400] m-0">
                                            <span className="text-[14px] text-primaryDark font-[700] leading-[28px] mr-[5px]">Ksh.{item.amountTargeted}</span>
                                            Goal
                                        </p>
                                    </div>
                                </div>
