  ...
  return (
    <section className='relative block pt-[60px] mt-[120px] pb-[90px] px-0 w-full'>
        <Layout className='flex flex-col items-center' >
            ...
            <nav className=''>
              <div className='flex w-full flex-wrap space-x-3 mb-[35px] text-center'>
                {['All', 'Education', 'medicine', 'Water', 'Food'].map((category) => (
                  <button
                  key={category}
                  onClick={() => handleFilterClick(category)}
                  className={`border-solid border-2 border-transparent capitalize font-bold bg-transparent py-[10px] px-[25px] text-darkBlue outline-none rounded-[30px] transition-all duration-500 ease-in-out ${activeFilter === category ? 'border-redDark' : 'nav-link'}`} 
                  >
                  {category}
                  </button>
                ))}
