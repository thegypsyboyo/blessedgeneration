      ...
      const handleMpesaSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
          const response = await fetch('/api/mpesa-payment', {
            ...
            body: JSON.stringify(formState), // Use formState for payment details
          });
      
          if (response.ok) {
            // Payment was successful, you can handle success actions here
            console.log('M-Pesa payment successful');
          } else {
            // Payment failed or encountered an error, handle error actions here
    ...
    const handleStripeSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (stripe) {
          const response = await fetch('/api/create-checkout-session', {
            ...
            body: JSON.stringify({
              ...formState,
            //   currency: selectedPaymentMethod === 'stripe' ? 'usd' : '',
