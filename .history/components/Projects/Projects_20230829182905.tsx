            ...
            <div className="flex flex-wrap w-full overflow-hidden">
                ...
                {filteredCauesItems.map((item, index) => (
                    <div className={` transition-all duration-500 w-full px-[15px] p-[15px] lg:max-w-[33.3333%] md:max-w-[50%] flex-grow-0 flex-shrink-0 group flex items-start`} key={index}>
                        <div className={`${activeFilter === 'All' || activeFilter === item.category ? 'transform translate-y-0 delay-500' : ''} w-full relative overflow-hidden block group bg-redDark h-[400px]`}>
                            <div className={`w-full h-full transform translate-y-[500px] delay-500 transition-all duration-500 ease-in-out`}>
