        ...
        <Layout className='flex flex-col items-center' >
            ...
            <nav className=''>
              <div className='flex flex-wrap items-center justify-center space-x-3 mb-[35px] text-center'>
                {['All', "Bg Ruiri", 'BG Malindi', 'Outreach ', "Bg Nyamira", "LightHouse Project"].map((category) => (
                  <button
                  key={category}
                            ...
                            <div className={`w-full h-full transform delay-500 transition-all duration-500 ease-in-out translate-y-0`}>
                                ...
                                <div className="mb-[11px] transition-all duration-[900]">
                                    {item.tags?.map((tag, index) => (
                                        <Link href={`/projects/project-details/${item.slug}`} className='text-white bg-yellowColor px-[15px] h-[24px] inline-block leading-[24px] text-[12px] font-semibold uppercase transition-all duration-700 hover:bg-redDark m-1' key={index}>{tag.title}</Link>
                                    ))}
                                </div>
